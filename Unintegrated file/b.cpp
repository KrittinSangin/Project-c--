...
if (!spinButtonTexture.loadFromFile("spinbutton.png"))
{
    cerr << "Failed to load spinbutton image!" << endl;
    return -1;
}
Sprite spinButton(spinButtonTexture);
float buttonCenterX = window.getSize().x / 2;
float buttonCenterY = window.getSize().y / 2;
spinButton.setPosition(320, 400);
spinButton.setScale(0.5f, 0.5f);
