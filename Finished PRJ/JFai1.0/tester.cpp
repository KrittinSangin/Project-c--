...
bool isCostClicked(int index, const sf::Event &event, store &STORE)
{
    ...
    if (STORE.forsale[index].soldOut)
...
void SpinSlot(int chips, char *RandomizedChar, vector<char> BaseChar, int vecsize, sf::RenderWindow &window, sf::Font &font, int maxchips, bool &FinishedPhase, coin &money, Player &player)
{
    ...
    Slot1.setFont(font);
    Slot1.setCharacterSize(65);
    Slot1.setFillColor(Color::Black);
    Slot1.setPosition(457, 180);

    Slot2 = Slot1;
    Slot2.setPosition(372, 180);

    Slot3 = Slot1;
    Slot3.setPosition(285, 180);
    ...
    ChipsText.setCharacterSize(32);
    ChipsText.setFillColor(Color::Black);
    ChipsText.setPosition(328, 312);
    ...
    if (!StoreBackgroundTexture.loadFromFile("spinning.jpg"))
    {
...
int main()
{
    const string filename = "SkillMem.txt";
    ...
    if (infile.is_open()) {
        ...
        if (outfile.is_open()) {
            cout << "File has been emptied.\n";
            outfile.close();
        } else {
            cerr << "Error opening file to empty it.\n";
        }
    } else {
    ...
    sf::Text button1Text("Spin", font, 30);
    button1Text.setFillColor(sf::Color::White);
    button1Text.setPosition(375, 410);
    ...
    while (window.isOpen())
    {
        ...
        if (gameState == SLOT_MACHINE)
        {
            ...
            if (MainmenuLoop)
            { // Show main menu if slot hasn't started yet
                window.draw(background);
        ...
        else if (gameState == NEXT_STAGE)
        {
            ...
            if(pragmaonce){
                for (int i = 0; i < player.availableSkills.size()   ; i++)
                {
                    output << player.availableSkills[i].name << endl;
                }
